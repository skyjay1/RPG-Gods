...
public class FavorRange {
  ...
  public ResourceLocation getDeity() { return deity; }
  ...
  public boolean isInRange(final PlayerEntity player) {
    if(player.isServerWorld() && player.getCapability(RPGGods.FAVOR).isPresent() && this != FavorRange.EMPTY) {
      return isInRange(player, player.getCapability(RPGGods.FAVOR).orElse(RPGGods.FAVOR.getDefaultInstance()));
    }
    return false;
  }
   ...
   */
  public boolean isInRange(final PlayerEntity player, final IFavor f) {
